...
//    * 움직이는 방향 반대편에 있는 점을 구하고 회전 변환 사용하여 상대좌표계에서의 위치 계산
//    */
...
//     }
//
//     return rotatePoint(fixedX, fixedY, centerX, centerY, rotate);
//   };
//
//   /**
...
//
//     // 상대좌표계에서 새로운 중심점 계산
//     const c = calculateNewCenter(fixedPoint, width, height);
//
...
//       0,
//       0,
//       -curPosition.rotate, // 회전 각도의 역수를 사용하여 원래 위치로 돌림
//     );
//
//     console.log(
//       'diff',
...
//
//     console.log('pointer down', curPosition.x, curPosition.y, startCenterX, startCenterY);
//
...
//     e.stopPropagation();
//     console.log('resize pointer down');
//
...
//     e.stopPropagation();
//     console.log('resize pointer up');
//
//     const newPosition = calculatePositionAfterResize(e);
//     console.log('pointer up', newPosition?.x, newPosition?.y);
//
...
const useResize = ({ cardId, layerId, type, children, curPosition, setCurPosition }: UseResizeProps) => {
  ...
  useEffect(() => {
    if (editorRef?.current && type === 'text') {
      ...
      setMinSize({ width: width + 30, height });
    }
  }, [children]);
