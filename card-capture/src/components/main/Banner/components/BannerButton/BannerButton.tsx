...
const BannerButton = () => {
  ...
  const variant = useExperiment({ experimentName: 'main-click', variants: ['original', 'simplified'] });
      ...
      {variant === 'original' ? (
        <div className="flex flex-row gap-4">
          {!isMobile && (
            <Link href={'/editor/sample'}>
              <div
                onClick={() => {
                  trackAmplitudeEvent('main-click-editor');
                }}
          ...
          <Link href={isLoggedIn ? '/prompt' : '/login?create=true'}>
            <div
              onClick={() => {
                trackAmplitudeEvent('main-click', { variant: 'original' });
              }}
        ...
        <div className="flex flex-row gap-4">
          <Link href={isLoggedIn ? '/prompt' : '/login?create=true'}>
            <div
              onClick={() => {
                trackAmplitudeEvent('main-click', { variant: 'simplified' });
              }}
              className="flex cursor-pointer flex-row items-center justify-center gap-1 rounded-[40px] bg-main px-[28px] py-[13px] md:px-[32px] md:py-[15px] lg:px-[38px] lg:py-[18px]"
              style={{ boxShadow: '0 0 14px rgba(111, 108, 255, 0.5)' }}
            >
